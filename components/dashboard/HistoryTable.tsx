...
export default function HistoryTable() {
  ...
  const { data, error, isLoading } = useSWR(
    isConnected && address ? `${rpcUrl}/api/v1/trade-history/${address}` : null,
    fetcher);
  useEffect(() => {
    if (data) {
  ...
  if (!isConnected) {
    return (
      <div className="w-full bg-[#1d1f22] rounded-[40px] flex flex-col">
        <div className="text-center text-[#888] py-8">Please connect your wallet to view trade history.</div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full bg-[#1d1f22] rounded-[40px] flex flex-col">
      ...
      {isLoading ? (
        <div className="text-center text-[#888] py-8">Loading trade history...</div>
          ...
          >
            <div className="flex-1 h-[63px] px-9 py-1.5 flex items-center gap-4">
              <div className="w-[38px] h-[38px] relative rounded-full overflow-hidden bg-[#2d2d2e] flex items-center justify-center">
                <Image
                  src={`https://flagcdn.com/w160/${item.countryId.toLowerCase()}.png`}
            ...
            <div className="flex-1 h-[63px] px-6 py-1.5 flex justify-center items-center">
              <motion.div
                className={`${item.realizedPnl ? 'text-[#00e431]' : 'text-[#f10000]'} text-xl font-normal font-['Inter'] leading-tight`}
                whileHover={{ scale: 1.05 }}
                transition={{ duration: 0.1, ease: [0.4, 0, 0.2, 1] }}
              >
                {item.realizedPnl} {item.realizedPnl ? '+' : '-'}{item.realizedPnl}%
              </motion.div>
            </div>

            <div className="flex-1 h-[63px] px-6 py-1.5 flex justify-center items-center">
              <div className={`text-xl font-normal font-['Inter'] leading-tight ${item.status === 'OPEN' ? 'text-[#00e431]' :
